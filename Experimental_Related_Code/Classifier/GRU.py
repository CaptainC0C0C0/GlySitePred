...
class GRU(nn.Module):
    ...
    def forward(self, x):
        x = x.unsqueeze(1)
        h0 = torch.zeros(1, x.size(0), self.hidden_size).to(x.device)  # Initialize GRU hidden state h0
        out, _ = self.gru(x, h0)  # Forward pass through GRU
        # Take the last time step output, pass through ReLU, then the fully connected layer
...
def train_model_kfold(data, labels, k=10, num_epochs=300, batch_size=32, lr=0.001):
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(data)):
        ...
        model = GRU(input_size, output_size).to(device)

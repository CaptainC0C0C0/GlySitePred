...
class LSTM(nn.Module):
    ...
    def forward(self, x):
        # Adjust input data format to (batch_size, seq_len, input_size)
...
def train_model_kfold(data, labels, k=10, num_epochs=300, batch_size=32, lr=0.001):
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(data)):
        ...
        model = LSTM(input_size, output_size).to(device)  # Using LSTM model
...
if __name__ == '__main__':
    ...
    data_file = '../ReSampling/NCRCC/esm2_features_train_NCRCC.csv'  # Path to training data CSV file
